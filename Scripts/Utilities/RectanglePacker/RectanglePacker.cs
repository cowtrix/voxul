...
namespace Voxul.Utilities.RectanglePacker
{
    public static class RectanglePacker
    {
        ...
        public static Dictionary<int, PackingRectangle> Pack(IEnumerable<Vector2> sizes, out PackingRectangle bounds,
            PackingHints packingHint = PackingHints.FindBest, float acceptableDensity = 1, uint stepSize = 1, uint padding = 0)
        {
            int idCounter = 0;
            var packingRectangles = new List<PackingRectangle>();
            foreach (var s in sizes)
            {
                var pr = new PackingRectangle(0, 0, (uint)Mathf.CeilToInt(s.x + padding), (uint)Mathf.CeilToInt(s.y + padding), idCounter);
                if(pr.Width > 0 && pr.Height > 0)
                {
                    packingRectangles.Add(pr);
                    idCounter++;
                }
            }
            if (!packingRectangles.Any())
            {
                bounds = default;
                return null;
            }
            var packedRects = packingRectangles.ToArray();
            Pack(packedRects, out bounds, packingHint, acceptableDensity, stepSize);
            return packedRects.ToDictionary(p => p.Id, p => p);
        }
        ...
        internal static object Pack(IEnumerable<Vector2> enumerable)
        {
